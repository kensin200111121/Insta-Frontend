...
import { apiEditLocation, apiUploadOwnershipProof } from '@/api/pages/location.api';
import { MyInputNumber, MySelect } from '@/components/basic';
import withDialog from '@/patterns/hoc/withDialog';
import { selectCurrencyOptions, selectReportTypeOptions, selectStateOptions } from '@/patterns/selectOptions';
...
const EditLocationDialog: React.FC<DialogContentProps<LocationCreateFormItem, LocationCreateFormItem>> = ({
  ...
  React.useEffect(() => {
    ...
    setIsLiveStore(data?.live ?? false);
    setAutoTime(data?.isAutoBatchTime ?? true);
    setTipMode(data.tipMode);
    setNoTip(data.noTip);
    setNoConvenienceFee(data.noConvenienceFee);
    setConvenienceFeeModes({
      percentageFeeMode: data.percentageFeeMode,
      fixedFeeMode: data.fixedFeeMode,
    });
    setProcessingFeeModes({
      percentageProcessingFeeMode: data.percentageProcessingFeeMode,
      fixedProcessingFeeMode: data.fixedProcessingFeeMode,
    });
    setHasAnotherAgent(data.secondAgent?.length ? true : false);
    setMerchants(data.merchants);
    setDescriptors(data.merchants.map(m => merchant_accounts.find(d => d._id == m)?.descriptor || ''));
  }, [data]);

  React.useEffect(() => {
    // setDescriptor(merchant_accounts.find(d => d._id == data?.merchant)?.descriptor || '');
  }, [data, merchant_accounts]);

  React.useEffect(() => {
    if (data.reportUsers) {
      setReportUsers([...data.reportUsers]);
    }

    if (data.userMembers) {
      setUserMembers([...data.userMembers]);
    }
  }, [data.reportUsers, data.userMembers]);

  React.useEffect(() => {
    userMembers.forEach((userMember, key) => {
      usersForm.setFieldValue(`name${key}`, userMember.name);
      usersForm.setFieldValue(`pin${key}`, userMember.pin);
    });

    reportUsers.forEach((reportUser, key) => {
      reportForm.setFieldValue(`name${key}`, reportUser.name);
      reportForm.setFieldValue(`email${key}`, reportUser.email);
      reportForm.setFieldValue(`phone${key}`, reportUser.phone);
      reportForm.setFieldValue(`type${key}`, reportUser.type);
    });
  }, [reportUsers, userMembers]);

  const [tipMode, setTipMode] = useState(data.tipMode);
  ...
  const handleTipModeChange = (mode: string) => {
    setTipMode(mode);
    setNoTip(false);
    form.setFieldsValue({ tipMode: mode });
  };
  ...
  const handleAddNewUser = async () => {
    ...
    if(userMembers.length){
      ...
      updatedLastMember = { ...lastMember, readOnly: false };
    }

    setUserMembers(prevMembers =>
        prevMembers
            .map((member, index) => (index === prevMembers.length - 1 ? updatedLastMember as UserMembersInterface : member))
            .concat({
                _id: '',
                pin: '',
                readOnly: false,
  ...
  const formSubmit = async (noTipConfirm: boolean, noConvenienceFeeConfirm: boolean) => {
    if(noConvenienceFeeConfirm === true){
  ...
  const handleClose = () => {
    const confirmExit = window.confirm("Any changes will not be saved, are you sure you want to exit?");
    if (confirmExit) {
      setUserMembers([]);
      setReportUsers([]);
      setTipMode('');
  ...
  const handleAutoTimeChange = (value: boolean) => {
    setAutoTime(value);
    form.setFieldsValue({ isAutoBatchTime: value });
  };

  useEffect(() => {
    form.setFieldsValue(data);
    data.tipAmounts.forEach((tipAmount, key) => {
      if (data.tipMode === 'percentage') {
        form.setFieldValue(`tipAmount${key + 1}`, tipAmount);
        form.setFieldValue(`tipAmount${key + 4}`, 0);
      } else {
        form.setFieldValue(`tipAmount${key + 4}`, tipAmount);
        form.setFieldValue(`tipAmount${key + 1}`, 0);
      }
    });
    form.setFieldsValue({ isAutoBatchTime: data.isAutoBatchTime });
  }, []);
    ...
    <div css={styles}>
      ...
      >
        ...
        <Row gutter={16} className='nested-row'>
          ...
          <Col span={5}>
            <Form.Item name="bankState" rules={[{ required: true, message: 'Please select the bank state!' }]}>
              <MySelect
                className="select-normal w-full"
                options={selectStateOptions}
                placeholder="State"
        ...
        <Row gutter={16} className='nested-row'>
          ...
          <Col span={5}>
            <Form.Item name="commissionRatesAmount">
              <MyInputNumber className='w-full' placeholder='Enter Amount' min={0} max={100} precision={2} addonAfter='%' />
            </Form.Item>
          </Col>
          <Col span={4}></Col>
          ...
          <Col span={5}>
            <Form.Item name="perTransactionAmount">
              <MyInputNumber className='w-full' placeholder='Enter Amount' min={0} max={1000} precision={2} addonBefore='$' css={css`.ant-input-number-input{padding-left: 3px}`}/>
            </Form.Item>
          </Col>
        </Row>
      ...
      <Form form={usersForm} name="basic" onFinish={handleSubmit} autoComplete="off" className="setting-form" style={{marginTop: '10px'}}>
        ...
        {userMembers.map((member, index) => (
          <Row gutter={16} key={index} className='nested-row'>
            ...
            <Col span={4}>
              <Button className="btn-red" onClick={() => handleDeleteMember(index)}>
                Delete
              </Button>
            </Col>
          </Row>
      ...
      >
        ...
        <Row className='subtitle-row'>
          ...
          <Col span={1}>
            <Form.Item name="live" >
              <Switch checked={isLiveStore} disabled />
            </Form.Item>
          </Col>
          <Col span={1}></Col>
...
export default withDialog(EditLocationDialog);
const styles = css`
  .btn-active {
    height: 42px;
    border-radius: 30px;
